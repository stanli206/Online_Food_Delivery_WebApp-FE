...
const PaymentSuccessPage = () => {
  ...
  useEffect(() => {
    ...
    const finishOrder = async () => {
      try {
        const order = await dispatch(confirmStripeOrder()).unwrap();
  ...
  return (
    <div className="mt-10 max-w-md mx-auto bg-white rounded-xl shadow p-6 text-center">
      ...
      <p className="text-xs text-gray-600 mb-4">
        We&apos;re confirming your order and preparing your food.
      </p>

      {confirmError ? (
        <div>
          <p className="text-sm text-red-600 mb-2">{confirmError}</p>
          <Link to="/orders" className="text-sm text-red-500 underline">
            Go to My Orders
          </Link>
        </div>
      ) : (
        <p className="text-sm text-gray-700">
          {confirmLoading
            ...
            : "Almost done... redirecting to your order details."}
        </p>
      )}
    </div>
  );
